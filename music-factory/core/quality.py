...
def vocabulary_report(conn, niche, *, threshold=0.4, top=25, protegidas=()):
    ...
    for _, texto in letras:
        for w in set(_content_words(texto)):
            if w not in prot:
                presenca[w] += 1
...
def title_collisions(conn, niche, *, min_overlap=2):
    ...
    sets = [(r["id"], r["title"], {w for w in _content_words(r["title"])}) for r in rows]
...
def avoid_list(conn, niche, *, n_palavras=15, n_rimas=5, protegidas=(), min_musicas=3):
    ...
    """
    voc = vocabulary_report(conn, niche, protegidas=protegidas)
...
def format_report(conn, niche, *, protegidas=()):
